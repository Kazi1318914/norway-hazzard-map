...
// ---------------------------------------------------------------------------
// Basemaps. Both keyless, which is the whole requirement here.
//
// The default stacks two rasters. Kartverket's topograatone is the Norwegian
// national map and by far the most detailed inside Norway, but it only covers
// Norway: ask for a tile over Stockholm or Helsinki and it returns 200 with a
// fully transparent PNG. On its own that leaves every neighbouring country
// blank, which reads as a broken map rather than a national one.
//
// Because those out-of-coverage tiles are transparent rather than white, the fix
// is to draw a global layer underneath. Norway keeps its detail, and Sweden,
// Finland and Denmark get context. Inside Norway the Kartverket tiles are also
// ~23% transparent over water, so the global layer shows through there and gives
// the sea a tone instead of a hole.
//
// CARTO's light_all used to be the default. It now stamps "API KEY REQUIRED /
// carto.com/basemaps/apikey" diagonally across every tile while still answering
// 200 with a valid PNG, so nothing throws and no console warning appears. It is
// removed rather than left as a broken option.
// ---------------------------------------------------------------------------
const ESRI_GRAY =
...
export const BASEMAPS = {
  kartverket: {
    label: "Kartverket + global context",
    // `under` renders beneath `tiles`, filling in everything outside Norway.
    under: [ESRI_GRAY],
    tiles: [KARTVERKET_GRAY],
    attribution: "\u00a9 Kartverket, \u00a9 Esri, \u00a9 OpenStreetMap contributors",
  },
  esri: {
    label: "Global only (Esri Light Gray)",
    tiles: [ESRI_GRAY],
